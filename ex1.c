...
void envio_string(char *string){
    ...
    *p_tx_buffer++ = 0xb3;
    *p_tx_buffer++ = strlen(string);

     for(int a = 0;string[a];a++){
    	*p_tx_buffer++ = string[a];
     }
    *p_tx_buffer++ = 1;
    *p_tx_buffer++ = 8;
    *p_tx_buffer++ = 0;
    *p_tx_buffer++ = 0;
    ...
    if (uart0_filestream != -1)
    {
	    ...
	    // printf("%d bytes received\n",rx_length);
            printf("Envia String(0xB3 + %s) - Retorno = %s\n",string , rx_buffer);
        }
    }
...
void envio_float(float number){
    ...
    unsigned char *p_tx_buffer;
    char digits[5];
    memcpy(digits,&number,sizeof(float));

     p_tx_buffer = &tx_buffer[0];
    *p_tx_buffer++ = 0xb2;
    *p_tx_buffer++ = digits[0];
    *p_tx_buffer++ = digits[1];
    *p_tx_buffer++ = digits[2];
    *p_tx_buffer++ = digits[3];
    *p_tx_buffer++ = 1;
    *p_tx_buffer++ = 8;
    *p_tx_buffer++ = 0;
    *p_tx_buffer++ = 0;
    ...
    if (uart0_filestream != -1)
    {
        ...
        else
        {
            // Bytes received
	    // printf("%d bytes received\n",rx_length);
            printf("Envia FLOAT (0xB2 + %f) - Retorno = %f\n",number , rx_buffer);
        }
    }
